...
const INITIAL_SPAWN_CHANCE = 0.2;
...
const GridAnimation = () => {
	...
	// --- Helpers ---------------------------------------------------------------
	const asciiWidth = useRef(
		Math.max(...DEMOS_ASCII.map((l) => l.trimEnd().length))
	); // ≈ 50
	...
	const computeStartX = (containerEl) => {
		...
		const visibleCols = Math.max(
			1,
			Math.min(GRID_WIDTH, Math.floor(containerEl.clientWidth / chPx))
		);
		...
		const maxX = Math.max(
			0,
			GRID_WIDTH - asciiWidth.current - HORIZONTAL_PADDING
		);
	...
	const createBackgroundGrid = (xStart) => {
		const grid = Array(GRID_HEIGHT)
			.fill(null)
			.map(() => Array(GRID_WIDTH).fill(EMPTY_CHAR));
	...
	const findLetterPositions = (grid) => {
		...
		grid.forEach((row, y) => {
			row.forEach((ch, x) => {
				if (re.test(ch)) arr.push({ x, y, char: ch });
			});
		});
		return arr;
	};
	...
	const spawnType1 = () => {
		if (
			type1Chars.length >= MAX_TYPE1_CHARS ||
			letterPositions.current.length === 0
		)
			return;
		const sourcePos =
			letterPositions.current[
				Math.floor(Math.random() * letterPositions.current.length)
			];
	...
	const updateCharacters = () => {
		...
		setType1Chars((prev) =>
			prev.map((char) => {
				...
				return {
					...
					speed: newSpeed,
					lastMove: now,
					movesSinceDirectionChange: movesSince,
					movesBeforeDirectionChange: movesBefore,
				};
			})
		);
		...
		if (type2Char) {
			setType1Chars((prev) => {
				const remaining = prev.filter(
					(c) => !(c.x === type2Char.x && c.y === type2Char.y)
				);
		...
		const spawnChance = isInitialSpawnPeriod
	...
	const renderGrid = () => {
		...
		if (
			type2Char &&
			type2Char.y >= 0 &&
			type2Char.y < GRID_HEIGHT &&
			type2Char.x >= 0 &&
			type2Char.x < GRID_WIDTH
		) {
			displayGrid[type2Char.y][type2Char.x] = TYPE2_CHAR;
			colored.set(`${type2Char.y}-${type2Char.x}`, {
				char: TYPE2_CHAR,
				color: CHASER_COLOR,
		...
		return displayGrid.map((row, y) => {
			...
			return (
				<div
					key={y}
					style={{ margin: 0, padding: 0, height: "var(--line-height)" }}
				>
					{rowEls}
				</div>
			);
